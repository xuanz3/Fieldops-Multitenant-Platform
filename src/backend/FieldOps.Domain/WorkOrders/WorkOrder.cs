...
public sealed class WorkOrder
{
        ...
        DateTimeOffset? createdAt = null)
    {
        ...
        if (Id == Guid.Empty)
        {
            throw new ArgumentException("Work order ID cannot be empty.", nameof(id));
        }

        TenantId = tenantId;
        CustomerId = customerId;
        Reference = DomainText.Reference(reference, nameof(reference));
        Title = DomainText.Required(title, nameof(title), 200);
        Description = DomainText.Optional(description, nameof(description), 4000);
        Priority = priority;
